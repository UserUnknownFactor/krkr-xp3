...
class XP3File(XP3FileEntry):
    ...
    def read(self, encryption_type='none', raw=False):
        ...
        if self.adler32:
            ...
            if checksum != self.adler32:
                if not self.silent:
                    print(f'! Checksum error. Expected 0x{hex(self.adler32)} got 0x{hex(checksum)}')
                return None
    ...
    def xor(output_buffer, adler32: int, encryption_type: str, use_numpy: bool = False):
        ...
        if numpy and use_numpy and ("xor_full" in enc_type or "xor_plain" in enc_type or "xor_bytes" in enc_type):
            key = None
            dt = dtype(uint8)
